...
public:
AggregateGammaSignature() : m_group_domain("secp256k1") { }
~AggregateGammaSignature() { }
...
std::vector<AggregateSig> aggregate(std::vector<AggregateSet> &agg_set) {
    ...
    m_z.clear();
    Botan::PointGFp pk;
    std::vector<std::pair<std::string, std::string>> t_list;
    ...
    for(auto &item : agg_set) {
        ...
        temp.msg = item.msg;
        temp.a = m_group_domain.point_multiply(m_group_domain.multiply_mod_order(item.z, d_inv), pk, m_group_domain.multiply_mod_order(hash(pk, temp.msg), d_inv));
        m_z = m_group_domain.mod_order(m_z + item.z);


        if(std::find(t_list.begin(), t_list.end(), std::make_pair(item.pk_pem, item.msg)) == t_list.end())
            t_list.emplace_back(std::make_pair(item.pk_pem, item.msg));

        if(std::find(a_list.begin(), a_list.end(), temp.a) == a_list.end())
            a_list.emplace_back(temp.a);
...
bool isUniqueElements(std::vector<AggregateSig> &agg_sig) {
    std::vector<std::pair<std::string, std::string>> t_list;
    std::vector<Botan::PointGFp> a_list;

    for(auto &item : agg_sig) {
        t_list.emplace_back(std::make_pair(item.pk_pem, item.msg));
        a_list.emplace_back(item.a);
    }

    std::sort(t_list.begin(), t_list.end(), sortString);
    auto it_t = std::unique(t_list.begin(), t_list.end());
    if(it_t != t_list.end())
        return false;

    std::sort(a_list.begin(), a_list.end(), sortPoint);
...
static bool sortPoint(Botan::PointGFp &fir, Botan::PointGFp &sec) {
    return fir.get_x() < sec.get_x();
}

static bool sortString(std::pair<std::string, std::string> &a, std::pair<std::string, std::string> &b) {
